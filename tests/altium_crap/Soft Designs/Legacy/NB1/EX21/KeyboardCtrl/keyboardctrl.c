...
#define ADDR_PS2_DAT_REG      0xE7
#define PS2_DAT_REG            (*(__sfr unsigned char *)0xE7)
#define PS2_DAT_REG_bit(x)     ((*(__sfr __bitstruct_t *)0xE7).__b ## x )

#define ADDR_PS2_CTRL_REG     0xEF
#define PS2_CTRL_REG           (*(__sfr unsigned char *)0xEF)
#define PS2_CTRL_REG_bit(x) ((*(__sfr __bitstruct_t *)0xEF).__b ## x )
...
__bit PS2_GetBusyFlag(void){
   return PS2_CTRL_REG_bit(0) ;
}

__bit LCD_GetBusyFlag(void){
...
void LCDDisplayChar(unsigned char p, unsigned char l, unsigned char c)
{
   unsigned char tmp ;
...
void LCDDisplayLine(char * str, unsigned char start, unsigned char len, unsigned char line)
{
    for (unsigned char i = start; i < (start + len); i++)
    {
...
__bit keyboard_sent_init_sequence;
__bit keyboard_ignore_next_key;
__bit keyboard_caps_on;
__bit keyboard_caps_sent;
__bit char_written    = 0;
