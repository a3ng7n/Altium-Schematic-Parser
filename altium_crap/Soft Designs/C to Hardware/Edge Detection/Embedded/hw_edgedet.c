...
|*                                        | -1 -2 -1 |     | -1 0 +1 |
|*
|*                      This module will be compiled into hardware
 */
...
// Cache (will go into ASP internal memory)
int32_t hw_cache0[600];
int32_t hw_cache1[600];
int32_t hw_cache2[600];

/********************************************************************
|*
|*  FUNCTION    : hw_abs
|*
|*  PARAMETERS  : val = signed value
|*
|*  RETURNS     : |val|
|*
|*  DESCRIPTION : Simple hardware implementation of abs() function
 */


inline int hw_abs( int val )
{
...
uint16_t hw_luma( uint16_t pixel )
{
...
void hw_find_edges( uint16_t * input, uint16_t * output, uint16_t width, uint16_t height, uint16_t xres, unsigned short divisor )
{
    unsigned x, y;
    unsigned index;
    signed prevpixel;
    ...
    signed curpixel;
    signed gx, gy, g;
    int32_t * west = hw_cache0;
    int32_t * northsouth = hw_cache1;
    int32_t * east = hw_cache2;
    ...
    for ( y = 0; y < height; y++ )
    {
        nextpixel = hw_luma(input[index]);
        curpixel = hw_luma(input[index+1]);  // Mirror!

        for ( x = 0; x < width; x++ )
        {
            prevpixel = curpixel;
            curpixel = nextpixel;
            nextpixel = ( x < width - 1 ) ? hw_luma(input[index + 1]) : prevpixel;
    ...
    for ( x = 1; x < width - 1; x++ )
    {
        ...
        switch ( ++cache_index )
        {
        case 1  : east = hw_cache0; break;
        case 2  : east = hw_cache1; break;
        default : east = hw_cache2; cache_index = 0;
        }
        east[0] = input[x+1];
